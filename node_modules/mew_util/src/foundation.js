...
//         return parse.call(JSON, code.toString("utf8").replace(/\/\/.(\r|\n|\r\n)/m, "\r\n").replace(/\/\*([^*]|[\r\n]|(\*+([^\*\/]|[\r\n])))*\*+\//, ""));
//     };
// })(JSON.parse);
...
var simpleMerge = function () {

    var result = {};

    copyAsArray(arguments).forEach(function (object) {
        if (isKindOf(object, Object)) {
...
var matchArguments = function (matches, fields, argumentsToMatch) {
    ...
    iterateObject(fields, function (key, value) {
        formalFields[key] = advancedMerge({
            "!noLogger": true,
            "!stringFields": "acceptTypes",
            "!arrayFields": "acceptTypes",
            "acceptTypes": {
...
var hashCodeSymbol = Symbol("hashCode");

var hash = function (object, recalculate) {

    if (isNull(object) ||
        isKindOf(object, Boolean) ||
        isKindOf(object, Number) ||
        isKindOf(object, RegExp)) {
        return hash(jsonize(object));
    } else if (isKindOf(object, Date)) {
        return hash(object.getTime());
    } else if (isKindOf(object, Symbol)) {
        return hash(object.toString());
    } else if (isKindOf(object, Function)) {

        if (!object.hasOwnProperty(hashCodeSymbol)) {
            Object.defineProperty(object, hashCodeSymbol, {
                "enumerable": false,
                "value": hash(object.toString())
            });
        }

        return object[hashCodeSymbol];

    } else if (isKindOf(object, Buffer)) {
        return hash(offset.toString("utf8"));
    } else if (isKindOf(object, String)) {
        ...
        var looper = 0;
        while (looper < object.length) {

            result = (((result << 5) - result) + object.charCodeAt(looper)) | 0;
    ...
    } else if (isKindOf(object, Array)) {

        if ((!recalculate) && object.hasOwnProperty(hashCodeSymbol)) {
            return object[hashCodeSymbol];
        }
        ...
        Object.keys(object).sort().filter(function (key) {
            return isKindOf(key, String)
        }).forEach(function (key){
            result = result ^ (hash(key) + hash(object[key]));
        });
