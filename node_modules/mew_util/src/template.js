...
//      -> template.run(template, call, parameters, options)
//      -> template.execute(template, parameters, options)
...
finalTemplate.Operator = function (template, parameters, options, namedArguments, definition) {
    ...
    this.call = function (callTemplate, call, callParameters, callOptions, overwriteParameters) {

        var callArguments = [].slice.call(arguments, 4);

        var newParameters = {};
        if (arguments.length === 5) {
            for (var name in overwriteParameters) {
                newParameters[name] = overwriteParameters[name];
            }
        } else {
            for (var name in parameters) {
                newParameters[name] = parameters[name];
        ...
        if (arguments.length === 5) {
            newParameters["^"] = overwriteParameters;
        } else {
        ...
        switch (charCode) {
            ...
            case 39:            // single quotation
            {
                ...
                while ((index2 < template.length) &&
                    ...
                    if (code == 92) {
                        ...
                        result.push(template.charAt(index2));
            ...
            case 125:           // }
            {
                ...
                if ((index + 1 < template.length) &&
                    (combinedOperators.indexOf(template.charAt(index) + template.charAt(index + 1)) != -1)) {
                    return {
                        "type": "opr",
                        "content": template.charAt(index) + template.charAt(index + 1),
                        "index": index,
                        "next": index + 2
                    };
                }
...
finalTemplate.functors["format"] = function (template, call, parameters, options, template, date) {
    ...
    while (looper < template.length) {
        ...
        switch (template.charAt(looper)) {

            case "Y": {

                if (template.charAt(looper + 1) == "Y") {

                    if ((template.charAt(looper + 2) == "Y") && (template.charAt(looper + 3) == "Y")) {
            ...
            case "S": {
                ...
                if ((template.charAt(looper + 1) == "S") && (template.charAt(looper + 2) == "S")) {
            ...
            case "'": {
                ...
                var offset = 1;
                while ((template.charAt(looper + offset) != template.charAt(looper)) &&
                       (looper + offset < template.length)) {

                    if (template.charAt(looper + offset) == "\\") {

                        result.push(template.charAt(looper + offset + 1));

                        offset += 2;

                    } else {

                        result.push(template.charAt(looper + offset));
            ...
            default: {
                ...
                result.push(template.charAt(looper));
...
finalTemplate.functors["index"] = function (template, call, parameters, options, list, item, reversed) {
    ...
    if (reversed) {
        ...
        while (looper > 0) {
            ...
            if (item instanceof finalTemplate.Operator) {
                if (item.call(template, call, parameters, options, looper, list[looper], list)) {
                    return looper;
                }
...
finalTemplate.functors["contain"] = function (template, call, parameters, options, list, item) {
...
finalTemplate.parsers["text/plain"] = function (template, parameters, options, cacher) {
    ...
    if (template.constructor === String) {
        ...
        while (looper < template.length) {
            ...
            if ((template.charAt(looper) == "$") &&
                (looper + 1 < template.length) &&
                (template.charAt(looper + 1) == "{")) {
        ...
        if (cacher) {
            ...
            cache.forEach(function (unit) {

                if (unit.type === "text") {
                    if ((simplfied.length > 0) &&
                        (simplfied[simplfied.length - 1].type === "text")) {
                        simplfied[simplfied.length - 1].text.push(unit.text);
                    } else {
                        simplfied.push({
                            "type": "text",
                            "text": [unit.text]
            ...
            simplfied.forEach(function (unit) {
                if (unit.type === "text") {
                    unit.text = unit.text.join("");
                }
            });
        ...
        template.forEach(function (cache) {

            if (cache.type === "text") {
                result.push(cache.text);
            } else if (cache.type === "call") {
                result.push(finalTemplate.run(cache.template, cache.call, parameters, options));
            } else {
                throw new Error("Failed to format the template with invalid cache");
            }
...
module.exports = {
    ...
    "execute": function () {
        if (arguments[0].constructor === String) {
            ...
            return finalTemplate.run.apply(finalTemplate, [arguments[0].template, arguments[0], arguments[1], arguments[2]]);
        }
    }
};
