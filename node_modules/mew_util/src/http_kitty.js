    ...
    if (kitty.status === WORKING || kitty.status === WAITING) {
        ...
        if (kitty.mewSendQueue.length) {
            ...
            async.delay(100, function() {
                ...
                kitty.logger.debug("sending mew to server : " + kitty.uri);

                kitty.mewSendQueue.sort(function(firstMew, secondMew) {
					return firstMew.time.getTime() - secondMew.time.getTime();
				});

                kitty.logger.debug(kitty.mewSendQueue);

                kitty.httpClient.request({
                    "method": "POST",
                    "headers": {
                        "mew-token": kitty.mewToken
                    },
                    "data": JSON.stringify(kitty.mewSendQueue),
                    "onSuccess": function(result,response) {

                        if (response.headers && response.headers["mew-token"]){
                            kitty.mewToken = response.headers["mew-token"];
                        }
...
var startListenFromRemote = function startListenFromRemote(kitty) {
    ...
    kitty.httpClient.request({
        ...
        "onSuccess": function(mews,response) {

            if (response.headers["mew-token"]){
                kitty.mewToken = response.headers["mew-token"];
            }
...
var HTTPKitty = function HTTPKitty(uri, options) {
    ...
    Object.defineProperty(this.mew, "rpc", {
        ...
        "value": function() {
            ...
            if (foundation.isKindOf(content,Object) && content['!content']) {
                rpcContent = content;
            } else {
                rpcContent = {
                    "!content": true
